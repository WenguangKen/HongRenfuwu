package com.athlunakms.shopify.order.controller;

import com.athlunakms.shopify.order.dto.BindInfluencerRequest;
import com.athlunakms.shopify.order.dto.ConversionOrderDto;
import com.athlunakms.shopify.order.dto.DuplicateCheckResult;
import com.athlunakms.shopify.order.dto.SampleOrderDto;
import com.athlunakms.shopify.order.dto.SyncProgressDto;
import com.athlunakms.shopify.order.dto.OrderFilterDto;
import com.athlunakms.shopify.order.entity.InfluencerConversionOrder;
import com.athlunakms.shopify.order.entity.InfluencerSampleOrder;
import com.athlunakms.shopify.order.entity.ShopifyOrder;
import com.athlunakms.shopify.order.repository.InfluencerConversionOrderRepository;
import com.athlunakms.shopify.order.repository.InfluencerSampleOrderRepository;
import com.athlunakms.shopify.order.repository.ShopifyOrderRepository;
...
public class InfluencerOrderController {
    ...
    private final com.athlunakms.shopify.store.repository.ShopifyStoreRepository storeRepository;
    private final com.athlunakms.shopify.order.repository.OrderLineItemRepository lineItemRepository;

    private final com.athlunakms.shopify.order.repository.InfluencerSampleOrderItemRepository sampleItemRepository;
    private final com.athlunakms.shopify.order.repository.InfluencerConversionOrderItemRepository conversionItemRepository;
    private final com.athlunakms.shopify.product.repository.ShopifyProductVariantRepository variantRepository;
    private final com.athlunakms.shopify.order.repository.InfluencerChangeLogRepository changeLogRepository;
    private final com.athlunakms.shopify.influencer.repository.InfluencerReadOnlyRepository influencerReadOnlyRepository;
    private final com.athlunakms.shopify.influencer.repository.SystemTagReadOnlyRepository systemTagReadOnlyRepository;
    ...
    private void logChange(Long influencerId, String fieldName, String oldVal, String newVal, String operator,
        ...
        if (influencerId == null)
            return;
        com.athlunakms.shopify.order.entity.InfluencerChangeLog logEntry = new com.athlunakms.shopify.order.entity.InfluencerChangeLog();
    ...
    public ResponseEntity<Map<String, Boolean>> checkOrderExists(@RequestParam(value = "orderNo") String orderNo) {
        // Check regular order number or shopify order number or name
        boolean exists = orderRepository.existsByName(orderNo) ||
                orderRepository.existsByShopifyOrderNumber(parseOrderNo(orderNo));
    ...
    public ResponseEntity<Page<SampleOrderDto>> getSampleOrders(
        ...
        Map<Long, List<com.athlunakms.shopify.order.entity.InfluencerSampleOrderItem>> itemsMap;
        if (!orderIds.isEmpty()) {
            List<com.athlunakms.shopify.order.entity.InfluencerSampleOrderItem> allItems = sampleItemRepository
                    .findBySampleOrderIdIn(orderIds);
            itemsMap = allItems.stream()
                    .collect(Collectors.groupingBy(
                            com.athlunakms.shopify.order.entity.InfluencerSampleOrderItem::getSampleOrderId));
        } else {
    ...
    public ResponseEntity<Page<SampleOrderDto>> searchSampleOrders(
        ...
        Specification<InfluencerSampleOrder> spec = createSampleSpec(filter.getOrderNo(), filter.getShopifyOrderId(), filter.getInfluencerName(), filter.getLogisticsStatus(), filter.getTab(),
        ...
        Map<Long, List<com.athlunakms.shopify.order.entity.InfluencerSampleOrderItem>> itemsMap;
        if (!orderIds.isEmpty()) {
            List<com.athlunakms.shopify.order.entity.InfluencerSampleOrderItem> allItems = sampleItemRepository.findBySampleOrderIdIn(orderIds);
            itemsMap = allItems.stream().collect(Collectors.groupingBy(com.athlunakms.shopify.order.entity.InfluencerSampleOrderItem::getSampleOrderId));
        } else {
            itemsMap = new HashMap<>();
        }
    ...
    public ResponseEntity<Map<String, Long>> searchSampleTabCounts(@RequestBody OrderFilterDto filter) {
        ...
        String effectivePackageNo = (filter.getPackageNo() != null && !filter.getPackageNo().isEmpty()) ? filter.getPackageNo() : filter.getTrackingNo();
        for (String tab : tabs) {
            Specification<InfluencerSampleOrder> spec = createSampleSpec(filter.getOrderNo(), filter.getShopifyOrderId(), filter.getInfluencerName(), filter.getLogisticsStatus(), tab,
                    effectivePackageNo, filter.getSpu(), filter.getTimeType(), filter.getStartTime(), filter.getEndTime(), filter.getInfluencerSocialSearch(), filter.getRecipientName(),
                    filter.getRecipientCountry(), filter.getRecipientCity(), filter.getCustomerEmail(), filter.getOwnerId(), filter.getContactPersonId());
            counts.put(tab, sampleOrderRepository.count(spec));
        }
    ...
    private Specification<InfluencerSampleOrder> createSampleSpec(String orderNo, String shopifyOrderIdFilter, String influencerName,
        ...
        return (root, query, cb) -> {
            ...
            // 订单号搜索：仅匹配 orderName 和 shopifyOrderNumber
            if (orderNo != null && !orderNo.trim().isEmpty()) {
                ...
                if (validOrderNos.size() > 1) {
                    ...
                    Predicate p2 = root.get("shopifyOrderNumber").as(String.class).in(validOrderNos);
                    ...
                    predicates.add(cb.or(
                            ...
                            cb.like(root.get("shopifyOrderNumber").as(String.class), pattern)
                    ));
                }
            }

            // 交易号搜索：仅匹配 shopifyOrderId
            if (shopifyOrderIdFilter != null && !shopifyOrderIdFilter.trim().isEmpty()) {
                String[] ids = shopifyOrderIdFilter.split("[\\n,]+");
                List<String> validIds = Arrays.stream(ids)
                        .map(String::trim)
                        .filter(s -> !s.isEmpty())
                        .collect(Collectors.toList());
                List<String> gidIds = validIds.stream()
                        .map(s -> s.matches("\\d{10,}") ? "gid://shopify/Order/" + s : s)
                        .collect(Collectors.toList());

                if (validIds.size() > 1) {
                    Predicate p1 = root.get("shopifyOrderId").in(gidIds);
                    Predicate p2 = root.get("shopifyOrderId").in(validIds);
                    predicates.add(cb.or(p1, p2));
                    ...
                    predicates.add(cb.or(
                            cb.like(root.get("shopifyOrderId"), pattern),
                            cb.like(root.get("shopifyOrderId"), gidPattern)
                    ));
                }
            }
            ...
            if (influencerName != null && !influencerName.isEmpty()) {
                ...
                if (!validNames.isEmpty()) {
                    ...
                    jakarta.persistence.criteria.Root<com.athlunakms.shopify.influencer.entity.InfluencerReadOnly> nameInfRoot = 
                        nameSub.from(com.athlunakms.shopify.influencer.entity.InfluencerReadOnly.class);
            ...
            if (packageNo != null && !packageNo.trim().isEmpty()) {
                ...
                if (validPkgs.size() > 1) {
                    ...
                    jakarta.persistence.criteria.Root<com.athlunakms.shopify.order.entity.InfluencerSampleOrderItem> itemRoot = itemSub
                            .from(com.athlunakms.shopify.order.entity.InfluencerSampleOrderItem.class);
            ...
            if (spu != null && !spu.trim().isEmpty()) {
                ...
                for (String term : validSpus) {
                    ...
                    try {
                        ...
                        idMatch = cb.or(
                            cb.equal(itemRoot.get("shopifyProductId"), spuId),
                            cb.equal(itemRoot.get("shopifyVariantId"), spuId)
                        );
                    } catch (Exception e) {}
            ...
            if ((ownerIds != null && !ownerIds.isEmpty()) || (contactPersonIds != null && !contactPersonIds.isEmpty())) {
                ...
                jakarta.persistence.criteria.Root<com.athlunakms.shopify.influencer.entity.InfluencerReadOnly> infRoot = 
                    infSub.from(com.athlunakms.shopify.influencer.entity.InfluencerReadOnly.class);
    ...
    public ResponseEntity<Map<String, Object>> bindSampleInfluencer(
        ...
        log.info("绑定红人请求: orderId={}, shopifyOrderNumber={}, influencerId={}",
                request.getOrderId(), request.getShopifyOrderNumber(), request.getInfluencerId());

        try {
            // 1. 根据订单号查找所有相关订单包括拆单
            Long orderNumber = request.getShopifyOrderNumber();
            if (orderNumber == null && request.getOrderId() != null) {
                // 通过orderId获取订单号
                ShopifyOrder order = orderRepository.findById(request.getOrderId()).orElse(null);
                if (order != null) {
                    orderNumber = order.getShopifyOrderNumber();
                }
            }
            ...
            // 3. 更新所有相关的ShopifyOrder
            List<ShopifyOrder> relatedOrders = orderRepository.findAllByShopifyOrderNumber(orderNumber);
            if (relatedOrders.isEmpty()) {
                return ResponseEntity.badRequest().body(Map.of("success", false, "message", "未找到对应的Shopify订单"));
            }

            for (ShopifyOrder order : relatedOrders) {
                ...
                if (oldInfluencerId != null && !oldInfluencerId.equals(request.getInfluencerId())) {
                    log.info("订单 {} 从红人 {} 换绑到 {}", order.getShopifyOrderNumber(), oldInfluencerId,
                            request.getInfluencerId());
                }
                ...
                if (oldInfluencerId != null && !oldInfluencerId.equals(request.getInfluencerId())) {
                    logChange(request.getInfluencerId(), "转移接收样品单", "-",
                            order.getName() + " (" + order.getShopifyOrderNumber() + ")",
                            operator != null ? operator : "SYS", "从红人ID:" + oldInfluencerId + " 转移");
                    logChange(oldInfluencerId, "转移送出样品单", order.getName() + " (" + order.getShopifyOrderNumber() + ")",
            ...
            List<InfluencerSampleOrder> relatedSampleOrders = sampleOrderRepository
                    .findByShopifyOrderNumber(orderNumber);

            if (relatedSampleOrders.isEmpty()) {
                // 如果不存在根据 ShopifyOrder 创建新的记录
                for (ShopifyOrder order : relatedOrders) {
                    // 通常每个 ShopifyOrder 对应一个 SampleOrder (或者主单)
                    // 这里我们只为每个唯一的 shopifyOrderId 创建一个记录避免重复
                    if (sampleOrderRepository.findByShopifyOrderId(order.getShopifyOrderId()).isEmpty()) {
                        InfluencerSampleOrder newSampleOrder = new InfluencerSampleOrder();
                        newSampleOrder.setShopifyOrderId(order.getShopifyOrderId());
                        newSampleOrder.setShopifyOrderNumber(order.getShopifyOrderNumber());
                        ...
                        // populate items
                        List<com.athlunakms.shopify.order.entity.OrderLineItem> sourceItems = lineItemRepository
                                .findByOrderId(order.getId());
                        if (!sourceItems.isEmpty()) {
                            List<com.athlunakms.shopify.order.entity.InfluencerSampleOrderItem> newItems = sourceItems
                                    .stream().map(src -> {
                                        com.athlunakms.shopify.order.entity.InfluencerSampleOrderItem item = new com.athlunakms.shopify.order.entity.InfluencerSampleOrderItem();
                                        item.setSampleOrderId(savedOrder.getId());
                                        item.setShopifyLineItemId(src.getShopifyLineItemId());
                                        item.setShopifyProductId(src.getShopifyProductId());
                                        item.setShopifyVariantId(src.getShopifyVariantId());
    ...
    public ResponseEntity<Page<ConversionOrderDto>> getConversionOrders(
        ...
        Map<Long, List<com.athlunakms.shopify.order.entity.InfluencerConversionOrderItem>> itemsMap;
        if (!orderIds.isEmpty()) {
            List<com.athlunakms.shopify.order.entity.InfluencerConversionOrderItem> allItems = conversionItemRepository
                    .findByConversionOrderIdIn(orderIds);
            itemsMap = allItems.stream()
                    .collect(Collectors.groupingBy(
                            com.athlunakms.shopify.order.entity.InfluencerConversionOrderItem::getConversionOrderId));
        } else {
    ...
    private Specification<InfluencerConversionOrder> createConversionSpec(Long influencerId, String influencerName,
        ...
        return (root, query, cb) -> {
            ...
            if (orderNo != null && !orderNo.trim().isEmpty()) {
                ...
                List<String> shopifyOrderIds = validOrderNos.stream()
                        .map(s -> s.matches("\\d{10,}") ? "gid://shopify/Order/" + s : s)
                        .collect(Collectors.toList());
                ...
                if (validOrderNos.size() > 1) {
                    Predicate p1 = root.get("orderName").in(validOrderNos);
                    Predicate p2 = root.get("shopifyOrderNumber").as(String.class).in(validOrderNos);
                    Predicate p3 = root.get("shopifyOrderId").in(shopifyOrderIds);
                    Predicate p4 = root.get("shopifyOrderId").in(validOrderNos);
                    predicates.add(cb.or(p1, p2, p3, p4));
                } else if (validOrderNos.size() == 1) {
                    String singleNo = validOrderNos.get(0);
                    String pattern = "%" + singleNo + "%";
                    String idPattern = "%" + shopifyOrderIds.get(0) + "%";
                    predicates.add(cb.or(
                            cb.like(root.get("orderName"), pattern),
                            cb.like(root.get("shopifyOrderNumber").as(String.class), pattern),
                            cb.like(root.get("shopifyOrderId"), idPattern),
                            cb.like(root.get("shopifyOrderId"), pattern)
                    ));
                }
            }
            ...
            if (packageNo != null && !packageNo.trim().isEmpty()) {
                ...
                if (validPkgs.size() > 1) {
                    Predicate trackingMatch = root.get("trackingNumber").in(validPkgs);
                    jakarta.persistence.criteria.Subquery<Long> itemSub = query.subquery(Long.class);
                    jakarta.persistence.criteria.Root<com.athlunakms.shopify.order.entity.InfluencerConversionOrderItem> itemRoot = itemSub
                            .from(com.athlunakms.shopify.order.entity.InfluencerConversionOrderItem.class);
    ...
    private List<SampleOrderDto> splitToSampleDtos(InfluencerSampleOrder entity,
            List<com.athlunakms.shopify.order.entity.InfluencerSampleOrderItem> items) {
        // 商品明细已传入无需再次查询
        // 返回单个 DTO (解耦逻辑简化拆分)
    ...
    private SampleOrderDto createBaseDto(InfluencerSampleOrder entity,
        ...
        dto.setShopifyOrderId(entity.getShopifyOrderId());
        dto.setShopifyOrderNumber(entity.getShopifyOrderNumber());
        dto.setOrderNo(entity.getOrderName());
        dto.setLongOrderNo(entity.getShopifyOrderId());
        ...
        // Timestamps
        dto.setProcessedAtShopify(entity.getProcessedAt());
        ...
        // Items mapping
        List<com.athlunakms.shopify.order.dto.OrderLineItemDto> productDtos = items.stream()
    ...
    private com.athlunakms.shopify.order.dto.OrderLineItemDto toSampleItemDto(
            com.athlunakms.shopify.order.entity.InfluencerSampleOrderItem item) {
        com.athlunakms.shopify.order.dto.OrderLineItemDto dto = new com.athlunakms.shopify.order.dto.OrderLineItemDto();
        dto.setId(item.getId());
        dto.setShopifyLineItemId(item.getShopifyLineItemId());
        dto.setShopifyProductId(item.getShopifyProductId());
        dto.setShopifyVariantId(item.getShopifyVariantId());
        ...
        if (image == null || image.isEmpty()) {
            if (item.getShopifyVariantId() != null) {
                image = variantRepository.findByShopifyVariantId(item.getShopifyVariantId())
                        .map(com.athlunakms.shopify.product.entity.ShopifyProductVariant::getImageUrl)
                        .orElse(null);
            }
    ...
    private List<ConversionOrderDto> splitToConversionDtos(InfluencerConversionOrder entity,
            List<com.athlunakms.shopify.order.entity.InfluencerConversionOrderItem> items) {
    ...
    private com.athlunakms.shopify.order.dto.OrderLineItemDto toConversionItemDto(
            com.athlunakms.shopify.order.entity.InfluencerConversionOrderItem item) {
        com.athlunakms.shopify.order.dto.OrderLineItemDto dto = new com.athlunakms.shopify.order.dto.OrderLineItemDto();
        dto.setId(item.getId());
        dto.setShopifyProductId(item.getShopifyProductId());
        dto.setShopifyVariantId(item.getShopifyVariantId());
