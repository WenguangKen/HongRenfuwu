...
import com.athlunakms.shopify.order.dto.OrderCreateRequest;
import com.athlunakms.shopify.order.dto.OrderCreateResponse;
...
/**
 * Shopify 订单创建与管理服务
 * 负责本地订单草稿的创建、确认以及通过 Shopify GraphQL API 创建真实订单
 */
@Slf4j
@Service
@RequiredArgsConstructor
public class ShopifyOrderCreateService {
    ...
    /**
     * 创建订单包含本地草稿流程和 Shopify 真实下单流程
     */
    @Transactional
    public OrderCreateResponse createOrder(OrderCreateRequest request) {
        log.info("执行订单创建: 店铺={}, 是否草稿={}", request.getStoreId(), request.getIsDraft());

        ShopifyStore store = storeRepository.findById(request.getStoreId())
        ...
        return handleShopifyOrderCreation(request, store);
    }

    private OrderCreateResponse handleLocalDraft(OrderCreateRequest request, ShopifyStore store) {
        String draftId = "gid://shopify/Order/draft_" + UUID.randomUUID().toString();
        String draftName = "DRAFT-"
                + LocalDateTime.now().format(java.time.format.DateTimeFormatter.ofPattern("yyyyMMdd-HHmmss"));

        java.util.Map.Entry<ShopifyOrder, List<OrderLineItem>> result = saveOrderFromCreate(request, store,
                extractLegacyId(draftId), 0L, draftName, "-", null, null);

        ShopifyOrder order = result.getKey();
    ...
    private OrderCreateResponse handleShopifyOrderCreation(OrderCreateRequest request, ShopifyStore store) {
        ...
        String apiDomain = isNonEmpty(store.getMyshopifyDomain()) ? store.getMyshopifyDomain()
                : encryptionService.decrypt(store.getStoreUrlEncrypted());
        ...
        try {
            ...
            validateShopifyResponse(body);

            JsonNode orderNode = body.path("data").path("orderCreate").path("order");
            String shopifyOrderId = orderNode.path("id").asText();
            ...
            Long shopifyOrderNumber = Long.valueOf(extractLegacyId(shopifyOrderId));

            java.util.Map.Entry<ShopifyOrder, List<OrderLineItem>> result = saveOrderFromCreate(request, store,
                    shopifyOrderId, shopifyOrderNumber, name, financialStatus, createdAt, lineItemsNode);

            orderClassificationService.classifyOrder(result.getKey(), result.getValue());
            return OrderCreateResponse.success(shopifyOrderId, shopifyOrderNumber, name, result.getKey().getId());

        } catch (Exception e) {
            log.error("Shopify 下单失败: {}", e.getMessage());
            throw new RuntimeException("Shopify API 调用失败: " + e.getMessage());
        }
    }

    @Transactional
    public OrderCreateResponse confirmDraft(Long draftOrderId) {
        ShopifyOrder draft = orderRepository.findById(draftOrderId)
    ...
    private void lockInventory(OrderCreateRequest request) {
        for (OrderCreateRequest.OrderLineItemRequest item : request.getLineItems()) {
            if (item.getVariantId() == null)
                continue;
            ShopifyProductVariant variant = variantRepository.findByIdWithLock(item.getVariantId())
    ...
    private ObjectNode buildOrderCreateGraphQL(OrderCreateRequest request, String defaultCurrency) {
        ...
        for (OrderCreateRequest.OrderLineItemRequest item : request.getLineItems()) {
            ...
            String variantGid = item.getShopifyVariantId() != null
                    ? "gid://shopify/ProductVariant/" + item.getShopifyVariantId()
                    : null;
            if (variantGid != null)
    ...
    private java.util.Map.Entry<ShopifyOrder, List<OrderLineItem>> saveOrderFromCreate(OrderCreateRequest request,
            ShopifyStore store, String shopifyOrderId, Long shopifyOrderNumber, String orderName,
    ...
    private List<OrderLineItem> saveLineItemsFromCreate(ShopifyOrder order, OrderCreateRequest request,
        ...
        for (OrderCreateRequest.OrderLineItemRequest itemReq : request.getLineItems()) {
            ...
            if (lineItemsNode != null && lineItemsNode.isArray() && index < lineItemsNode.size()) {
                ...
                item.setShopifyLineItemId(extractLegacyId(n.path("id").asText()));
            ...
            if (item.getShopifyLineItemId() == null) {
                item.setShopifyLineItemId("draft_" + UUID.randomUUID().toString());
            }

            if (itemReq.getVariantId() != null) {
                variantRepository.findById(itemReq.getVariantId()).ifPresent(v -> {
                    item.setShopifyVariantId(v.getShopifyVariantId());
                    item.setShopifyProductId(v.getProductId());
                    if (item.getSku() == null)
    ...
    private OrderCreateRequest mapEntityToRequest(ShopifyOrder order) {
        ...
        req.setLineItems(items.stream().map(i -> {
            ...
            ir.setShopifyVariantId(i.getShopifyVariantId() != null ? String.valueOf(i.getShopifyVariantId()) : null);
    ...
    public OrderCreateResponse updateDraft(Long id, OrderCreateRequest request) {
        ...
        ShopifyOrder draft = orderRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("未找到草稿订单"));
        ...
        ShopifyOrder savedOrder = orderRepository.save(draft);
        ...
        return OrderCreateResponse.success(savedOrder.getShopifyOrderId(), savedOrder.getShopifyOrderNumber(),
    ...
    private void validateShopifyResponse(JsonNode body) {
