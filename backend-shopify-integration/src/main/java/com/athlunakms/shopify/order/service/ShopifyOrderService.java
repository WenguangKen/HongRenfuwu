...
import com.athlunakms.shopify.order.dto.*;
 ...
 * 提供统一的订单操作入口内部逻辑委派给 ShopifySyncService 和 ShopifyOrderCreateService
 */
@Slf4j
@Service
...
public class ShopifyOrderService {
    ...
    private final ShopifyOrderCreateService orderCreateService;
    ...
    /**
     * 指定时间范围的异步同步（按 Shopify 「更新时间」，供定时任务增量同步）
     */
    ...
    /**
     * 管理端「订单同步」弹窗：与 Shopify Admin 一致按「更新时间 updated_at」筛选时间范围，并 forceUpdate=true，
     * 避免本地已有记录时因「Shopify 更新时间未变」被跳过；不臆测改为 created_at（会与「近期有变动的老单」场景冲突）。
     */
    ...
    public void bindInfluencerToOrder(String orderId, Long influencerId, String operator) {
        orderRepository.findByShopifyOrderId(orderId).ifPresent(order -> {
     ...
     * 获取所有订单列表按创建时间倒序
     */
    public List<ShopifyOrderDto> getAllOrders() {
        return orderRepository.findAllByOrderByCreatedAtShopifyDesc().stream()
     ...
     * 获取指定店铺的订单列表
     */
    public List<ShopifyOrderDto> getOrdersByStoreId(Long storeId) {
        return orderRepository.findByStoreIdOrderByCreatedAtShopifyDesc(storeId).stream()
                .map(this::convertToDto)
                .collect(Collectors.toList());
    }
     ...
     * 获取订单详情包含行项目、交易和退款记录
     */
    public ShopifyOrderDto getOrderById(Long id) {
    ...
    private ShopifyOrderDto convertToDto(ShopifyOrder entity) {
        ShopifyOrderDto dto = new ShopifyOrderDto();
        BeanUtils.copyProperties(entity, dto);
        return dto;
    }

    private ShopifyOrderDto convertToDtoWithDetails(ShopifyOrder entity) {
        ShopifyOrderDto dto = convertToDto(entity);
        ...
        // 加载交易记录
        List<OrderTransaction> transactions = transactionRepository.findByOrderIdOrderByCreatedAtShopifyDesc(entity.getId());
        dto.setTransactions(transactions.stream().map(this::convertTransactionToDto).collect(Collectors.toList()));

        // 加载退款记录
        List<OrderRefund> refunds = refundRepository.findByOrderIdOrderByCreatedAtShopifyDesc(entity.getId());
    ...
    private void calculateFinancialStats(ShopifyOrderDto dto, List<OrderTransaction> txs) {
    ...
    private OrderRefundDto convertRefundToDto(OrderRefund entity) {
        ...
        if (entity.getCreatedAtShopify() != null) {
            dto.setTime(entity.getCreatedAtShopify().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
        }
