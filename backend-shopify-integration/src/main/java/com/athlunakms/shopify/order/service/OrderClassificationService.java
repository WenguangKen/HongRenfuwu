...
import com.athlunakms.shopify.influencer.repository.InfluencerReadOnlyRepository;
...
public class OrderClassificationService {
    ...
    private final com.athlunakms.shopify.order.repository.ShopifyOrderRepository shopifyOrderRepository;
     ...
     * 将 shopify_orders 中的 delivered_at 和 fulfillment_display_status 回填到业务表
     * 同时处理 fulfillment_display_status=delivered 但 delivered_at 为空的情况
     */
    @Transactional
    public void repairDeliveredStatus() {
        try {
            // 1. 从 shopify_orders 回填 delivered_at 到转化订单
            ...
            // 2. 从 shopify_orders 回填 delivered_at 到样品单
     ...
     */
    @Transactional
    public void syncBusinessTable(ShopifyOrder order,
            java.util.List<com.athlunakms.shopify.order.entity.OrderLineItem> lineItems) {
        ...
        if (sampleOrderRepository.existsByOrderId(order.getId())) {
            ...
            sampleOrderRepository.findByOrderId(order.getId()).ifPresent(sample -> {
                ...
                sample.setCancelledAt(order.getCancelledAtShopify());
                sample.setOrderCreatedAt(order.getCreatedAtShopify()); // 已同步创建时间
        ...
        if (conversionOrderRepository.existsByOrderId(order.getId())) {
            ...
            conversionOrderRepository.findByOrderId(order.getId()).ifPresent(conversion -> {
                ...
                if (conversion.getInfluencerId() == null) {
                    log.error(
                            "ABORTING SAVE: InfluencerId is null for ConversionOrder ID: {}, ShopifyOrder: {}. This prevents SQL 1048 error.",
     ...
     */
    @Transactional
    public void forceReclassify(ShopifyOrder order, List<OrderLineItem> lineItems) {
        if (order == null || order.getId() == null) {
            return;
        }
     ...
     */
    @Transactional
    public void manualBindInfluencer(ShopifyOrder order,
            java.util.List<com.athlunakms.shopify.order.entity.OrderLineItem> lineItems) {
        if (conversionOrderRepository.existsByOrderId(order.getId())) {
     ...
     */
    @Transactional
    public void classifyOrder(ShopifyOrder order,
            java.util.List<com.athlunakms.shopify.order.entity.OrderLineItem> lineItems) {
        if (order == null || order.getId() == null) {
            return;
        }
     ...
     * 判断是否为样品单
     */
    private boolean isSampleOrder(ShopifyOrder order) {
        // 1. 检查标签
     ...
     * 创建或更新样品单
     */
    private void createOrUpdateSampleOrder(ShopifyOrder order,
            java.util.List<com.athlunakms.shopify.order.entity.OrderLineItem> lineItems) {
        if (sampleOrderRepository.existsByOrderId(order.getId())) {
        ...
        sample.setShopifyOrderNumber(order.getShopifyOrderNumber());
     ...
     */
    private InfluencerMatch matchInfluencerByDiscountCode(ShopifyOrder order) {
     ...
     * 创建或更新转化订单
     */
    private void createOrUpdateConversionOrder(ShopifyOrder order, InfluencerMatch match,
            java.util.List<com.athlunakms.shopify.order.entity.OrderLineItem> lineItems) {
        ...
        conversion.setShopifyOrderId(order.getShopifyOrderId());
        conversion.setShopifyOrderNumber(order.getShopifyOrderNumber());
        ...
        conversion.setCancelledAt(order.getCancelledAtShopify());
        conversion.setProcessedAt(order.getProcessedAtShopify());
        conversion.setPaidAt(order.getProcessedAtShopify()); // 支付时间使用 processedAt
        conversion.setOrderCreatedAt(order.getCreatedAtShopify());
    ...
    private void updateConversionOrderAmounts(InfluencerConversionOrder conversion, ShopifyOrder order) {
    ...
    /**
     * 更新样品单的物流信息
     * 以 ShopifyOrder 的直接字段为权威数据源JSON仅用于提取物流追踪信息
     */
    private void updateFulfillmentInfo(InfluencerSampleOrder sample, ShopifyOrder order) {
        // 1. 物流状态和送达时间始终从 ShopifyOrder 直接字段获取权威数据源
        ...
        sample.setClosedAt(order.getClosedAtShopify());
     ...
     * 以 ShopifyOrder 的直接字段为权威数据源JSON仅用于提取物流追踪信息
     */
    private void updateFulfillmentInfo(InfluencerConversionOrder conversion, ShopifyOrder order) {
        // 1. 物流状态和送达时间始终从 ShopifyOrder 直接字段获取权威数据源
        ...
        conversion.setClosedAt(order.getClosedAtShopify());
     ...
     */
    private void applyAutoRepairLogic(Object businessOrder, ShopifyOrder order) {
    ...
    /**
     * 从 ShopifyOrder 的 fulfillmentsJson 解析第一个 Fulfillment 的物流信息
     * REST API 返回的 fulfillments 数组结构
     * [{ "status": "success", "tracking_company": "UPS", "tracking_number": "123",
     * "tracking_url": "...", ... }]
     */
    private FulfillmentInfo parseFulfillmentInfo(ShopifyOrder order) {
        ...
        try {
            ...
            if (fulfillments.isArray() && fulfillments.size() > 0) {
                ...
                // 应该信任 Shopify 的实际 fulfillment_display_status
     ...
     * 根据地址文本去重，避免重复录入相同地址
     */
    private void syncOrderAddressToInfluencer(Long influencerId, ShopifyOrder order) {
        if (influencerId == null) return;
    ...
    private void syncSampleOrderItems(Long sampleOrderId, Long shopifyOrderId,
            java.util.List<com.athlunakms.shopify.order.entity.OrderLineItem> sourceItems) {
        sampleItemRepository.deleteBySampleOrderId(sampleOrderId);

        // 获取订单状态以判断是否已取消
        var order = sampleOrderRepository.findByOrderId(shopifyOrderId).orElse(null);
        boolean isCancelled = order != null && order.getCancelledAt() != null;

        if (sourceItems != null && !sourceItems.isEmpty()) {
            var newItems = sourceItems.stream().map(src -> {
                var item = new com.athlunakms.shopify.order.entity.InfluencerSampleOrderItem();
    ...
    private void syncConversionOrderItems(Long conversionOrderId, Long shopifyOrderId,
            java.util.List<com.athlunakms.shopify.order.entity.OrderLineItem> sourceItems) {
        conversionItemRepository.deleteByConversionOrderId(conversionOrderId);

        // 获取订单状态以判断是否已取消
        var order = conversionOrderRepository.findByOrderId(shopifyOrderId).orElse(null);
        boolean isCancelled = order != null && order.getCancelledAt() != null;

        if (sourceItems != null && !sourceItems.isEmpty()) {
            var newItems = sourceItems.stream().map(src -> {
                var item = new com.athlunakms.shopify.order.entity.InfluencerConversionOrderItem();
