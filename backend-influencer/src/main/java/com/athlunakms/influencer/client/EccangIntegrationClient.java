...
@Component
public class EccangIntegrationClient {
    private static final Logger log = LoggerFactory.getLogger(EccangIntegrationClient.class);
    private final RestTemplate restTemplate;
    @Value(value="${services.eccang-integration.url:http://localhost:8081}")
    private String eccangIntegrationUrl;
    ...
    public Map<Long, Integer> getSampleOrderCounts(List<Long> influencerIds) {
        ...
        try {
            String url = UriComponentsBuilder.fromUriString((String)(this.eccangIntegrationUrl + "/api/eccang/v1/influencer-orders/stats/sample-counts")).queryParam("influencerIds", influencerIds.toArray()).toUriString();
            log.debug("Calling eccang-integration service: {}", (Object)url);
            ...
            log.warn("Failed to get sample order counts from eccang-integration service: {}", (Object)e.getMessage());
            return Collections.emptyMap();
        }
    }
    ...
    public Map<Long, Integer> getConversionOrderCounts(List<Long> influencerIds) {
        ...
        try {
            String url = UriComponentsBuilder.fromUriString((String)(this.eccangIntegrationUrl + "/api/eccang/v1/influencer-orders/stats/conversion-counts")).queryParam("influencerIds", influencerIds.toArray()).toUriString();
            log.debug("Calling eccang-integration service: {}", (Object)url);
            ResponseEntity<Map<Long, Integer>> response = this.restTemplate.exchange(url, HttpMethod.GET, null, new ParameterizedTypeReference<Map<Long, Integer>>() {}, new Object[0]);
            return response.getBody() != null ? response.getBody() : Collections.emptyMap();
        }
        catch (Exception e) {
            log.warn("Failed to get conversion order counts from eccang-integration service: {}", (Object)e.getMessage());
            return Collections.emptyMap();
        }
    }
    ...
    public void bindInfluencer(String orderNo, Long influencerId, String influencerName) {
        try {
            String url = this.eccangIntegrationUrl + "/api/eccang/v1/influencer-orders/bind";
    ...
    public boolean checkOrderExists(String orderNo) {
        try {
            String url = UriComponentsBuilder.fromUriString((String)(this.eccangIntegrationUrl + "/api/eccang/v1/influencer-orders/check-order")).queryParam("orderNo", new Object[]{orderNo}).toUriString();
    ...
    public EccangIntegrationClient(RestTemplate restTemplate) {
