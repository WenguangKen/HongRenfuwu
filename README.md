...
| 易仓集成服务 | `backend-eccang-integration/` | ✅ 必需（支持易仓商品同步与红人样品/转化订单模块） |
...
- **商品同步**：易仓 API → `backend-eccang-integration` → `eccang_products`/`eccang_product_variants` 表（支持签名自动刷新机制、多变体解析与图片路径同步）
- **订单管理**：`backend-eccang-integration` 提供红人样品单与转化单的完整逻辑处理与导入/发货机制：
  - 样品单支持创建到易仓通过 FBA 发货，或通过订单 ID 单单导入与自动内容抓取
  - 转化单支持通过 Excel 文件批量导入，自动去重样品单，必须绑定红人 Handle 以汇总佣金统计
- **红人业务**：`backend-influencer` 负责档案、多社交媒体绑定、佣金结算、内容审核与汇款打款等
...
| 文件 | 内容 | 适用场景 |
|------|------|----------|
| `database_scripts/hongrenfuwu.sql` | 数据库全表 DDL（含订单表、导入日志表、权限表） + 基础初始数据 | 新环境建库与初始数据注入 |
...
```bash
mysql -h <DB_HOST> -u root -p hongrenfuwu < database_scripts/hongrenfuwu.sql
```
...
| 订单 | `orders`, `influencer_sample_order`, `influencer_sample_order_item`, `influencer_conversion_order`, `influencer_conversion_order_item`, `influencer_conversion_import_log` |
