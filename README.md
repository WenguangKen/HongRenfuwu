...
> **Influencer Marketing System** — 面向电商商家的一站式红人 CRM、样品单/转化单、Shopify 订单同步、佣金结算与内容资产管理平台。

| 项目 | 说明 |
|------|------|
| 系统名称 | 红人营销系统 |
| 数据库名 | `hongrenfuwu` |
| 当前版本 | 后端 user `1.0.0` · influencer/shopify `1.0.4` · webhook `1.0.0` · 前端 `1.0.0` |
| 最后更新 | 2026-05 |
...
| Shopify 集成服务 | `backend-shopify-integration/` | ✅ 必需 |
| Webhook 接收服务 | `backend-webhook/` | ✅ 必需 |
...
| 发布辅助 | `release-manager/` | 可选（内部运维面板，非业务必需） |
...
| **Shopify 店铺凭证** | 在系统「设置 → Shopify」中配置，不入库到代码仓库 |
...
```
                         ...
                         └──┬────────┬─────────┬──────────┬────┘
                            │        │         │          │
              /api/*        │ /influencer-api/* │ /api/shopify/* │ /api/webhook/*
                            ▼        ▼         ▼          ▼
                    ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐
                    │ backend- │ │ backend- │ │ backend- │ │ backend- │
                    │   user   │ │influencer│ │ shopify  │ │ webhook  │
                    │  :8080   │ │  :8082   │ │  :8081   │ │  :8083   │
                    └────┬─────┘ └────┬─────┘ └────┬─────┘ └────┬─────┘
                         │            │            │            │
                         └────────────┴─────┬──────┴────────────┘
...
- **订单同步**：Shopify GraphQL 定时/手动拉取 → `backend-shopify-integration` → `orders` 表 → 分类为样品单/转化单
- **实时 webhook**：Shopify → `backend-webhook` → 更新订单与业务表
...
| 外部集成 | Shopify Admin API (GraphQL/REST) · Shopify Webhooks |
...
├── backend-shopify-integration/  # Shopify 订单/商品/折扣同步
├── backend-webhook/              # Shopify Webhook 接收与订单处理
...
├── release-manager/              # 可选：微服务发布辅助面板
├── docker-compose.yml            # 本地开发：MySQL + Redis + MinIO
├── docker-compose.prod.yml       # 生产：4 后端 + Redis + Nginx 前端
...
| backend-shopify-integration | athluna-shopify-integration | `athluna-shopify-integration-1.0.4.jar` |
| backend-webhook | athluna-webhook | `athluna-webhook-1.0.0.jar` |
...
| Shopify | `*.myshopify.com` | 订单/商品/折扣 |
...
cd backend-shopify-integration
mvn spring-boot:run
...
├── backend-shopify-integration/app.jar
├── backend-webhook/app.jar
...
| influencer-backend-shopify-prod | Shopify 集成 | 8081 | 内部 |
| influencer-backend-influencer-prod | 红人业务 | 8082 | `/influencer-api/actuator/health` |
| influencer-backend-webhook-prod | Webhook | 8083 | 内部 |
...
| `/api/shopify/*` | `backend-shopify:8081` | 订单同步、商品、折扣 |
| `/api/webhook/*` | `backend-webhook:8083` | Shopify Webhook 入口 |
...
| Shopify | `shopify_stores`, `shopify_products`, `shopify_discounts` |
...
| `SHOPIFY_INTEGRATION_URL` | influencer 调 shopify 服务 | `http://backend-shopify:8081` |
| `USER_SERVICE_URL` | influencer 调 user 服务 | `http://backend-user:8080` |

Shopify 店铺 Access Token、Webhook Secret 等在**系统管理界面**配置，存入 `shopify_stores` / `system_configs` 表（加密存储）。
...
| 商品 | `/product/list` | Shopify 商品同步 |
| 佣金配置 | `/commission/*` | 分佣规则、佣金单、打款 |
| 汇款 | `/finance/remittance` | 红人汇款任务 |
| 内容库 | `/content/*` | 待审核 / 内容库 |
| 系统管理 | `/system/*` | 用户、角色、标签、Shopify、Webhook、存储 |
...
# 重启单个服务
docker compose -f docker-compose.prod.yml restart backend-shopify
...
- **定时同步**：`backend-shopify-integration` 每 30 分钟增量同步（`updated_at` 窗口 2 小时）
- **手动同步**：系统内「订单同步」弹窗，按时间范围 force 更新
- **Webhook**：Shopify 事件 → `backend-webhook` → 实时更新
...
- [ ] 确认 4 个后端可 `mvn clean package -DskipTests` 通过
...
- [ ] 提供初始管理员账号说明（或首次登录引导）
- [ ] 说明 Shopify App / Webhook URL 注册方式
...
├── 源码（本仓库全部目录）
├── README.md（本文档）
├── 部署凭据说明.pdf（单独密送，含 DB/Redis/SSH/Shopify）
├── 数据库脚本/
...
1. 检查 `shopify_stores` 中店铺状态为 `active`
2. 查看 `backend-shopify` 日志是否有 GraphQL 错误
3. 确认 Shopify Access Token 未过期
4. Webhook 需在 Shopify 后台指向 `https://<域名>/api/webhook/...`
